...
- **Live markdown formatting**: while you type the note shows the markdown
  source; about a second after you stop it renders itself formatted in place
  (headings, bold/italic/code, lists…). Click the rendered text to go back to
  editing. A **Format** toolbar inserts markdown around the selection.
...
- **Key backup**: a single **Key Backup** footer button opens **Export /
  ...
  restores a backup into the notes folder before the first unlock.
...
- **Key Backup** — opens **Export** / **Import**. Export saves the
  `.quicknote-seal` (folder picker); Import restores a backup into the notes
  folder — do this **before the first Unlock on a new machine** (see
...
3. Use **Key Backup → Export** and store a copy outside the machine (the salt
   is not in the repo on purpose).
4. To restore on a new machine: clone/install, use **Key Backup → Import** and
   pick the backed-up seal (or copy it into the notes folder as
   `.quicknote-seal`), **then** Unlock with the password and run Sync.
...
- Crypto Notes does **not** watch the remote: sync is manual and directional.
  Edit on machine A, press **Sync** there (push); on machine B press **Sync**
  to pull — do that **after** A pushed.
...
- Encrypted notes: on a new machine use **Key Backup → Import** to restore the
  `.quicknote-seal` **before the first unlock**, otherwise the list can be
  empty (a fresh salt is created).
- If Sync reports unrelated histories, follow the command it prints
  (`git fetch origin && git merge --allow-unrelated-histories …`) and Sync
  again.
- Open the **Sync** window to read the log when notes do not appear — a failed
  fetch/merge shows the reason there.

## Quick-open shortcut (Ctrl+Alt+Enter)

The shortcut is a **system keybinding** per machine (in
`~/.config/hypr/bindings.lua`) — it is not part of the plugin install. On a
new machine, open the **?** help inside Crypto Notes and press **Enable
shortcut**; it adds the binding and reloads Hyprland for you.
