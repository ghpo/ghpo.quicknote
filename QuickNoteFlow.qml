...
Item {
  ...
  // First-run encryption question.
  property bool onboardOpen: false
  ...
  function enableQuickOpenShortcut() {
    ...
    Quickshell.execDetached([root.omarchyPath + "/bin/omarchy-notification-send",
      "Shortcut enabled", "Ctrl+Alt+Enter will now open Crypto Notes"])
  }
  ...
  function onMarkChecked(exitCode) {
    // 1 = marker absent -> first run without encryption.
    if (exitCode === 1 && !root.cryptoEnabled) root.onboardOpen = true
  }
  ...
  function enableEncryptionNow() {
    ...
    root.onboardOpen = false
    root.writeOnboardMarker()
    Quickshell.execDetached([root.omarchyPath + "/bin/omarchy-notification-send",
      saved ? "Encryption enabled" : "Encryption requested",
      saved ? "Run omarchy-restart-shell to apply it." : "Add \"encryption\": true to shell.json, then omarchy-restart-shell."])
  }
        ...
        Item {
          ...
          BorderSurface {
            ...
            ColumnLayout {
              ...
              Text {
                Layout.fillWidth: true
                text: "Notes can be encrypted before they touch disk (XChaCha20-Poly1305 + Argon2id). You set a password on first unlock and the key stays only in memory."
              ...
              Text {
                Layout.fillWidth: true
                text: "To enable later, add \"encryption\": true to the ghpo.quicknote entry in ~/.config/omarchy/shell.json and run omarchy-restart-shell."
                color: Qt.darker(root.foreground, 1.7)
                font.family: root.fontFamily
                font.pixelSize: Style.font.caption
                wrapMode: Text.WordWrap
